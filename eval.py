...
def evaluate(data_folder, test_set_image_coco_ids_file, checkpoint, beam_size=1):
  """
  Evaluation
  ...
  # DataLoader
  _, _, test_images_split = getImageIndicesSplitsFromFile(
  ...
  for i, (image, caps, caplens, allcaps) in enumerate(
    ...
    while True:
      ...
      if step > 50: #TODO parameterize (with max_caption_len?)
        break
      step += 1
    ...
    img_captions = list(
      map(lambda c: [w for w in c if w not in {word_map[TOKEN_START], word_map[TOKEN_END], word_map[TOKEN_PADDING]}],
          img_caps))
    references.append(img_captions)

    # Hypotheses
    hypotheses.append([w for w in seq if w not in {word_map[TOKEN_START], word_map[TOKEN_END], word_map[TOKEN_PADDING]}])
...
if __name__ == '__main__':
  evaluate(data_folder='/home/mitja/datasets/coco2014_preprocessed',
