...
def count_adjective_noun_pairs(
    ...
    for i, coco_id in enumerate(tqdm(imgIds)):
        ...
        images[coco_id] = {}
        images[coco_id][PAIR_OCCURENCES] = 0
        ...
        for caption in decoded_captions:
            ...
            if combination_is_present:
                print(caption)
                images[coco_id][PAIR_OCCURENCES] += 1
    ...
    for n in range(1, 5):
        noun_occurences = len([d for d in images.values() if d[NOUN_OCCURRENCES] >= n])
        adjective_occurences = len(
            [d for d in images.values() if d[ADJECTIVE_OCCURRENCES] >= n]
        )
        pair_occurences = len([d for d in images.values() if d[PAIR_OCCURENCES] >= n])
