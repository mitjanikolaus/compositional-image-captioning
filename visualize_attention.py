...
def visualize_attention(image_path, encoded_caption, alphas, word_map, smooth=True):
  ...
  for t in range(len(decoded_caption)):
    if t > 50:
      break
    plt.subplot(np.ceil(len(decoded_caption) / 5.), 5, t + 1)

    plt.text(0, 1, '%s' % (decoded_caption[t]), color='black', backgroundcolor='white', fontsize=12)
    plt.imshow(image)
...
def check_args(args):
  ...
  parser.add_argument('-I', '--image',
                      help='Path to the image', required=True)
  ...
  parser.add_argument('-B', '--beam_size', default=5, type=int, help='beam size for beam search')
  parser.add_argument('--dont_smoothen', dest='smoothen', action='store_false', help='Do not smoothen alpha overlay')
